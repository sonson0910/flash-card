...
if (!workerSource.includes("VERIFY_IMPORT_INTENT") || !workerSource.includes('importClaimedAt')) fail('worker import intent verification and one-time claim are missing.');
...
if (!appHookSource.includes('readPendingBrowserExtensionImport') || appHookSource.includes("hash.includes('lf-import='")) fail('web app hook must load verified pending imports, not raw URL hashes.');
