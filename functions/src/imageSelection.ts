...
export type UnsplashPhoto = { alt_description?: unknown; description?: unknown; urls?: Record<string, unknown> };

const trustedUnsplashUrl = (value: unknown): value is string => {
  if (typeof value !== 'string') return false;
  try {
    const url = new URL(value);
...
export const selectRelevantUnsplashImage = (photos: UnsplashPhoto[], query: string): string | null => {
  const queryTokens = imageSearchTokens(query);
  const ranked = photos.map((photo, index) => {
    const descriptionTokens = new Set(imageSearchTokens(typeof photo.alt_description === 'string' ? photo.alt_description : (typeof photo.description === 'string' ? photo.description : '')));
    const score = queryTokens.reduce((total, token) => total + (descriptionTokens.has(token) ? 1 : 0), 0);
    const candidate = photo.urls?.regular ?? photo.urls?.full ?? photo.urls?.small;
    return { url: trustedUnsplashUrl(candidate) ? candidate : null, score, index };
  }).filter((candidate): candidate is { url: string; score: number; index: number } => Boolean(candidate.url))
    .sort((left, right) => right.score - left.score || left.index - right.index);
