...
def _render_skills_registry(skills: list[dict]) -> str:
    lines: list[str] = []
    for skill in skills:
...
def _render_catalog_yaml(skills: list[dict]) -> str:
...
def _render_catalog_markdown(skills: list[dict]) -> str:
    cats = group_by_category(skills)
    active = [c for c in CATEGORY_ORDER if cats.get(c)]
    lines = [
        "# Skills Catalog", "",
...
def _write_outputs_atomically(outputs: list[tuple[Path, str]]) -> None:
    for destination, _ in outputs:
        ...
        if destination.exists() and not destination.is_file():
            raise OSError(f"Output path is not a file: {destination}")
...
def _generated_outputs(skills: list[dict], repo_root: Path) -> list[tuple[Path, str]]:
    return [
        (repo_root / ".claude" / "scripts" / "skills_data.yaml", _render_skills_registry(skills)),
        (repo_root / "guide" / "SKILLS.yaml", _render_catalog_yaml(skills)),
        (repo_root / "guide" / "SKILLS.md", _render_catalog_markdown(skills)),
    ]


def write_generated_catalogs(skills: list[dict], repo_root: Path) -> tuple[Path, Path, Path]:
    outputs = _generated_outputs(skills, repo_root)
    _write_outputs_atomically(outputs)
    registry, catalog_yaml, catalog_markdown = (destination for destination, _ in outputs)
    return registry, catalog_yaml, catalog_markdown


def write_skills_registry(skills: list[dict], repo_root: Path) -> Path:
    out = repo_root / ".claude" / "scripts" / "skills_data.yaml"
    _write_outputs_atomically([(out, _render_skills_registry(skills))])
    return out


def write_catalog_yaml(skills: list[dict], repo_root: Path) -> Path:
    out = repo_root / "guide" / "SKILLS.yaml"
    _write_outputs_atomically([(out, _render_catalog_yaml(skills))])
    return out


def write_catalog_markdown(skills: list[dict], repo_root: Path) -> Path:
    out = repo_root / "guide" / "SKILLS.md"
    _write_outputs_atomically([(out, _render_catalog_markdown(skills))])
    return out
...
def main() -> None:
    ...
    registry, cat_yaml, cat_md = write_generated_catalogs(skills, repo_root)
