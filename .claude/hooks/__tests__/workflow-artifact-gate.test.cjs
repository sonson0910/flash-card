...
test('stage detector maps prompts and bash commands', () => {
  ...
  assert.equal(detectCommandStage('npx firebase-tools deploy --only hosting'), 'deploy');
  assert.equal(detectCommandStage('npm --prefix . exec firebase-tools -- deploy --only hosting'), 'deploy');
  assert.equal(detectCommandStage('gh workflow run deploy-production.yml'), 'deploy');
});
