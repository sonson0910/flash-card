...
function detectCommandStage(command) {
  ...
  if (!text) return null;
  const productionDecision = classifyCommand(text);
  const protectedDispatch = productionDecision?.decision === 'ask'
    && productionDecision.target !== 'unclassifiable shell execution';
  if (productionDecision?.target === 'firebase deploy' || protectedDispatch) return 'deploy';
