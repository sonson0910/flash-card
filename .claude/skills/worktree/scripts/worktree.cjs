...
function getMainWorktreePath(gitRoot, cwd, worktrees = []) {
  const gitCommonDir = getGitCommonDir(cwd);
  if (gitCommonDir) {
    const configResult = git(
      ['config', '--file', path.join(gitCommonDir, 'config'), '--get', 'core.worktree'],
      { silent: true, cwd },
    );
  ...
  const listedMainWorktree = worktrees.find(worktree => !worktree.bare);
  return listedMainWorktree ? path.resolve(listedMainWorktree.path) : gitRoot;
}
...
function getWorktreeRecords(gitRoot, cwd) {
  ...
  const listedWorktrees = parseWorktreeListPorcelain(result.output);
  return parseWorktreeListPorcelain(result.output, {
    gitCommonDir: getGitCommonDir(cwd),
    mainWorktreePath: getMainWorktreePath(gitRoot, cwd, listedWorktrees)
  });
}
...
function cmdPrune() {
  ...
  const pruneResult = git(pruneArgs, { silent: true });
