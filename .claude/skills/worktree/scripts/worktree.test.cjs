...
function createIsolatedRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'worktree-test-'));
  execSync('git init --quiet', { cwd, stdio: ['ignore', 'pipe', 'pipe'] });
  execSync('git config user.name WorktreeTest', { cwd, stdio: ['ignore', 'pipe', 'pipe'] });
  execSync('git config user.email worktree-test@example.com', { cwd, stdio: ['ignore', 'pipe', 'pipe'] });
  fs.writeFileSync(path.join(cwd, 'README.md'), '# fixture\n');
  execSync('git add README.md && git commit --quiet -m fixture && git branch -M main', {
    cwd,
...
test('create dry-run uses an explicitly detected isolated base branch', () => {
  const cwd = createIsolatedRepository();
  try {
    const info = run('info --json', { cwd });
    assert(info.success, 'Isolated repository info should succeed');
    const baseBranch = assertJSON(info.output).baseBranch;
    assert(baseBranch === 'main', `Expected isolated main base branch, got ${baseBranch}`);

    const result = run(`create test-explicit-base --dry-run --json --base ${baseBranch}`, { cwd });
    assert(result.success, 'Dry-run should succeed with detected explicit base');
    const json = assertJSON(result.output);
    assert(json.wouldCreate.baseBranch === baseBranch, 'Should use detected explicit base branch');
...
test('status identifies the original checkout as main when invoked from a linked worktree', () => {
  const repository = createIsolatedRepository();
  const linkedWorktree = `${repository}-linked-'quoted`;
  try {
    execFileSync(
      'git',
      ['worktree', 'add', '--quiet', '-b', 'linked-fixture', linkedWorktree],
      { cwd: repository, stdio: ['ignore', 'pipe', 'pipe'] },
    );

    const result = run('status --json', { cwd: linkedWorktree });
    assert(result.success, result.stderr || 'Status should succeed from linked worktree');
    const json = assertJSON(result.output);
    const original = json.worktrees.find(worktree => path.resolve(worktree.path) === path.resolve(repository));
    const linked = json.worktrees.find(worktree => path.resolve(worktree.path) === path.resolve(linkedWorktree));
    assert(original?.isMainWorktree === true, 'Original checkout should remain the main worktree');
    assert(original?.isCurrentWorktree === false, 'Original checkout should not be current');
    assert(linked?.isMainWorktree === false, 'Linked checkout should not be marked main');
    assert(linked?.isCurrentWorktree === true, 'Linked checkout should be current');
  } finally {
    try {
