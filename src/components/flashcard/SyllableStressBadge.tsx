...
export const SyllableStressBadge = React.memo(function SyllableStressBadge({
  ...
  return (
    ...
    >
      <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400 mr-0.5">
        Syllables:
      </span>
          ...
          return (
            <button
              ...
              className={`group relative flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs transition-all duration-200 cursor-pointer ${
                isStressed
                  ? 'border-cyan-400/70 bg-gradient-to-r from-cyan-500/25 to-cyan-400/15 text-cyan-300 font-black shadow-[0_0_12px_rgba(6,182,212,0.35)] ring-1 ring-cyan-400/40 hover:scale-105 active:scale-95'
                  : 'border-white/12 bg-white/5 text-slate-300 font-bold hover:border-cyan-400/40 hover:text-white hover:bg-white/10 active:scale-95'
              }`}
              title={isStressed ? `Primary stress: ${syl.text}` : `Syllable: ${syl.text}`}
            >
              {isStressed && (
                <span className="text-[11px] font-black leading-none text-cyan-400" aria-hidden="true">
                  ˈ
                </span>
              )}
              <span>{syl.text}</span>
              <Volume2
                size={10}
                className="opacity-0 group-hover:opacity-100 transition-opacity text-cyan-300 shrink-0"
                aria-hidden="true"
              />
            </button>
          );
