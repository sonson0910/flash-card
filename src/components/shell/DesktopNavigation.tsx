...
export function DesktopNavigation({
  ...
  return (
    <nav ref={navigationRef} aria-label="Primary" className={`${isPracticeView(viewMode) ? 'hidden' : 'flex'} app-navigation liquid-glass mx-3 mt-3 min-h-16 relative rounded-full px-3 md:mx-6 md:px-5 items-center justify-between flex-shrink-0 z-20 border border-white/15 backdrop-blur-2xl transition-colors`}>
      ...
      <div data-gsap-header-actions className="flex min-h-11 items-stretch gap-1.5 sm:gap-2.5">
        <div className="relative flex items-center">
          {syncIdentity.status === 'loading' ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={onSignOut} className="flex items-center justify-center min-w-11 min-h-11 w-11 h-11 rounded-full border border-emerald-500/40 bg-emerald-500/15 text-emerald-300 hover:bg-rose-500/20 hover:text-rose-300 transition-all duration-300 hover:scale-105 cursor-pointer shadow-md overflow-hidden" title="Sign out of cloud sync" aria-label="Sign out of cloud sync">
            ...
            <button
              ...
              className="flex min-h-11 items-center gap-1.5 rounded-full bg-cyan-400 px-4 py-1.5 text-xs font-extrabold text-[#071014] shadow-md shadow-cyan-500/25 transition-all duration-300 hover:scale-[1.04] hover:bg-cyan-300 active:scale-[0.98] cursor-pointer disabled:opacity-60 disabled:cursor-wait"
            ...
            <div className="flex min-h-11 items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-[11px] font-bold text-white/70" title="Cloud sync is unavailable. Data is saved on this device only.">
        ...
        <div className="flex min-h-11 shrink-0 items-center overflow-hidden rounded-full border border-white/15 bg-white/5 p-0.5 shadow-sm backdrop-blur-xl">
          <button type="button" onClick={toggleSound} className="flex size-10 shrink-0 items-center justify-center rounded-full text-white/80 transition-colors hover:bg-white/10 hover:text-white" aria-label={soundActive ? 'Mute sound effects' : 'Enable sound effects'} title={soundActive ? 'Mute sound effects' : 'Enable sound effects'}>
          ...
          <button type="button" onClick={onToggleTheme} className="flex size-10 shrink-0 items-center justify-center rounded-full text-white/80 transition-colors hover:bg-white/10 hover:text-white" aria-label={isDarkMode ? 'Use light theme' : 'Use dark theme'} title={isDarkMode ? 'Use light theme' : 'Use dark theme'}>
