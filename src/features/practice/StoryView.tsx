...
export function StoryView({ story, loading, onGenerate, onClose }: StoryViewProps) {
  ...
  return (
    ...
    <div className="max-w-2xl mx-auto flex flex-col items-center pt-8" aria-busy={loading}>
      ...
      {loading ? (
        <motion.div ref={loadingStatusRef} tabIndex={-1} role="status" aria-live="polite" aria-atomic="true" initial={{ opacity: 0, scale: 0.98, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: motionDurations.emphasis, ease: motionEase }} className="mx-auto flex h-[300px] w-full max-w-sm flex-col items-center justify-center rounded-[32px] border border-[var(--sf-border)] bg-[var(--sf-surface)] shadow-xl focus:outline-none"><Loader2 size={48} className="animate-spin text-[var(--sf-brand-text)] mb-4" aria-hidden="true" /><p className="font-bold text-[var(--sf-text)] text-center">Creating your story…</p></motion.div>
      ) : story ? (
        <motion.div initial={{ opacity: 0, scale: 0.98, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: motionDurations.emphasis, ease: motionEase }} className="w-full rounded-[32px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-8 text-[var(--sf-text)] shadow-2xl" aria-labelledby="story-heading">
