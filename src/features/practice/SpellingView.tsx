...
export function SpellingView({ cards, currentIndex, input, checked, correct, score, showResults, onInput, onCheck, onNext, onRestart, onClose }: SpellingViewProps) {
  ...
  return (
    ...
    <div className="max-w-2xl mx-auto flex flex-col items-center pt-8">
      {showResults ? (
        <motion.div initial={{ opacity: 0, scale: 0.98, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: motionDurations.emphasis, ease: motionEase }} className="w-full max-w-md rounded-[32px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-8 text-center text-[var(--sf-text)] shadow-2xl" aria-labelledby="spelling-results-heading">
        ...
        <div className="w-full">
          ...
          <motion.div key={currentIndex} variants={getStepVariants(1)} initial="enter" animate="center" exit="exit" transition={{ duration: motionDurations.emphasis, ease: motionEase }} onAnimationComplete={definition => { if (definition === 'center' && currentIndex > 0 && !checked) questionHeadingRef.current?.focus(); }} className="relative mb-6 flex flex-col items-center overflow-hidden rounded-[32px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-8 text-[var(--sf-text)] shadow-xl" aria-labelledby="spelling-question-heading">
            ...
            <AnimatePresence>{checked && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: motionDurations.standard, ease: motionEase }} className="w-full mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-[var(--sf-surface-raised)]"><div aria-hidden="true">{correct ? <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-2"><CheckCircle2 size={17} /> Correct</span> : <span className="text-sm font-bold text-rose-600 dark:text-rose-400 flex items-center gap-2"><XCircle size={17} /> The answer is <span className="underline font-black">{card.word}</span></span>}</div><button data-color-role="primary" ref={nextButtonRef} type="button" onClick={onNext} className="min-h-11 px-6 py-2.5 bg-[var(--sf-brand)] font-bold text-xs text-[var(--sf-on-brand)] rounded-xl flex items-center gap-1 hover:bg-[var(--sf-brand-hover)] hover:text-white transition-colors">{currentIndex === cards.length - 1 ? 'View results' : 'Next'} <ChevronRight size={14} aria-hidden="true" /></button></motion.div>}</AnimatePresence>
          </motion.div>
          </AnimatePresence>
