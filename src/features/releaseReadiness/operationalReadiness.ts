...
export function decideCanary(sample: CanarySample): {
  ...
  const values = Object.values(sample).filter((value): value is number => value !== undefined);
  if (values.some(value => !Number.isFinite(value) || value < 0)) throw new TypeError('Invalid canary evidence.');
  const reasons: string[] = [];
  const required = [sample.syncLossRate, sample.quotaUsageRate, sample.costRate];
  if (required.some(value => value === undefined)) return { action: 'hold', reasons: ['missing-evidence'] };
...
export interface StagingSmokeEvidence {
  ...
  readonly catalogCacheControl: string;
...
export function evaluateStagingSmoke(evidence: StagingSmokeEvidence): {
  ...
  if (!evidence.catalogCacheControl.includes('immutable')) reasons.push('catalog-cache');
