...
const renderSelectionIconSites = sites => {
  ...
  for (const pattern of Array.isArray(sites) ? sites : []) {
    ...
    remove.addEventListener('click', () => { void removeSelectionIconSite(pattern); });
...
const removeSelectionIconSite = async pattern => {
  try {
