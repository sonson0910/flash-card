...
(() => {
  const {
    APP_ORIGIN, DEFAULT_APP_URL, IMPORT_PROTOCOL_VERSION, IMPORT_PROTOCOL_V3, MAX_CONTEXT_LENGTH, DECK_METADATA_STORAGE_KEY, DECK_METADATA_RETIRED_SCOPES_STORAGE_KEY, extensionApi, transientStorage, deckMetadataStorage, usesPromiseApi,
  ...
  const quickAdd = async input => {
    ...
    const job={v:IMPORT_PROTOCOL_V3,id,ticket:createIntentId(),text:s.text,context:bounded(s.context,MAX_CONTEXT_LENGTH),sourceLanguage:'en',mode:'silent',sourceTabId:s.sourceTabId,workerTabId:null,anchor:s.anchor,createdAt:Date.now(),...(requestedDeck ? { requestedDeck } : {})};
    ...
    const verification = (async () => {
      ...
      return {
        ...
        intent: intent.v === IMPORT_PROTOCOL_V3
          ? { v: IMPORT_PROTOCOL_V3, id: job.id, text: job.text, context: bounded(job.context,MAX_CONTEXT_LENGTH), createdAt: job.createdAt, mode: 'silent', ticket: job.ticket, ...(typeof job.requestedDeck === 'string' && job.requestedDeck ? { requestedDeck: job.requestedDeck.slice(0, 128) } : {}) }
          : intent,
      };
    })();
  ...
  const handle = async (m,sender) => {
    ...
    if(type==='GET_DECKS')return{ok:true,decks:(await readDeckMetadata())?.decks ?? []};
