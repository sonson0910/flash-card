...
- Placement check using 6–12 items from an installed published tiered catalog,
...
- Script-aware normalization/scoring for Latin, Han/Kana and Hangul profiles;
...
2. Existing bounded Practice Pool is the only learner-card input. Daily planning
   never starts an unbounded library scan and never uses catalog installation as
...
6. Today becomes the default view. Paths maps to Catalog, Vocabulary maps to the
   existing Library and Progress opens the existing Insights surface. Legacy
...
- `/` resolves to Today.
- `?view=today`, `?view=catalog`, and `?view=library` are canonical shell states.
...
4. Continue Review reuses the existing due-only FSRS Study path; Phase 5 does not
...
7. Latin, Han/Kana and Hangul scoring has explicit tests for normalization,
   punctuation/spacing and wrong-script answers; fallback scoring is strict.
8. Placement uses 6–12 unique items, reports insufficient evidence honestly and
   recommends Foundation/Core/Advanced without mutating FSRS, mastery, XP or tiers.
...
11. URL navigation preserves unrelated parameters/hash and browser history restores
    Today/Paths/Vocabulary without stale lesson state.
